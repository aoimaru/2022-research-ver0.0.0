...
def get_corpus(args):
    ...
    if args.source == "github":
        if args.run == 1:
            corpus = W2VFunction.get_training_github_data_filter_run()
        else:
            corpus = W2VFunction.get_training_github_data()
    else:
        if args.run == 1:
            corpus = W2VFunction.get_training_gold_data_filter_run()
        else:
            corpus = W2VFunction.get_training_gold_data()

def main(args):
    corpus = get_corpus(args)
    BaseW2V.do(corpus, sg=args.sg, size=args.size, min_count=args.min_count, window=args.window, name=args.source, run=args.run)
