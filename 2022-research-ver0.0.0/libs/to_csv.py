...
ROOT_PATH = Config.ROOT_PATH + "/data/"
...
class ToCSV():
    @staticmethod
    def do(parameter, data):
        file_path = "{root_path}/{version}/{target}/{top_lim}/{seed}/sg-{sg}.size-{size}.min_count-{min_count}.window-{window}.run-{run}.model".format(
            root_path=ROOT_PATH,
            version=version,
            target,
            top_lim,
